...
class DashboardController extends Controller
{
    public function index(){
        ...
        $totalProgressTasks = Task::query()->where('status', 'in progress')->count();
        $myProgressTasks = Task::query()->where('status', 'in progress')
        ->where('assigned_user_id', $user->id)->count();
        ...
        $activeTasks = Task::query()->whereIn('status', ['pending', 'in progress'])->where('assigned_user_id', $user->id)->limit(10)->get();
